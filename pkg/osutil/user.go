...
// returns a *user.User for a row if that row's has the given value at the
// given index.
...
func LookupGroupFrom(groupFile, groupname string) (*user.Group, error) {
...
func LookupGroupIdFrom(groupFile, id string) (*user.Group, error) {
	f, err := os.Open(groupFile)
	if err != nil {
		return nil, err
	}
...
func LookupUserFrom(userFile, username string) (*user.User, error) {
...
func LookupUserIdFrom(userFile, uid string) (*user.User, error) {
	f, err := os.Open(userFile)
	if err != nil {
		return nil, err
	}
